...
def parse_detail(html: str, max_chars: int = 6000) -> dict:
    """Extract the notice body plus the signals that explain an empty body.

    Dorm notices are frequently posted as screenshots with an HWP attachment,
    leaving `body` empty. `image_count` and `attachments` let the caller tell
    "parser broke" apart from "the post genuinely has no text", and give the
    extractor something to reason about (filenames often carry the deadline).
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    return {
        ...
        "image_count": len(node.select("img")) if node else 0,
